...
在插件设置 → 节假日导入与管理 中设置 ICS 来源：

```
https://holiday.ailcc.com/api/holiday/ics
```

或使用 vault 中的 ICS 文件（相对路径）：
```
folder/holidays.ics
```

> 注意：不支持隐藏文件夹（`.xxx/`）中的文件

**ICS 日历说明**（以 https://holiday.ailcc.com/api/holiday/ics 为例）：
- 返回当前年份前后各两年的节假日（如 2026 年返回 2024-2028 年的数据）
- 包含法定节假日、调休放假、补班信息
- 数据随国务院通知更新
- 详细 API 文档：https://holiday.ailcc.com/about

导入方式：
- **自动导入**：开启后，从插件开启后的下一年开始，每年1月1号后首次启动自动导入当年节假日
- **手动导入**：设置页面点击按钮，或命令面板搜索"导入节假日"。**新用户需手动导入一次**，重复导入直接覆盖，而且只覆盖节假日部分
...
A: 请确认以下两点：
1. 当前查看的是导入数据对应的年份（导入 2026 年数据需切换到 2026 年视图）
2. 在显示设置中已勾选「节日」选项

**Q: 需要每年都导入吗？**

A: 不需要。导入后的节假日数据会持久保存，每年自动应用。
...
A: 可以，只要 ICS 文件格式正确即可。不同来源的节假日可能使用不同的分类标签。
